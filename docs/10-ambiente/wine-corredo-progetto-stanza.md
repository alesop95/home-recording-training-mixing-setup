...
Quella che era una ipotesi è stata confermata in interfaccia il 2026-09-15: il catalogo vive nella base di dati e si alimenta per importazione, non copiando file in una cartella. L'importazione però non riesce, ed è il blocco corrente della sottofase 8.6, descritto nella sezione seguente.

### Il blocco corrente: l'importazione dei modelli rifiuta ogni file

Al 2026-09-15 nessun modello è importabile. Ogni tentativo termina con lo stesso avviso, `The following file is no System Definition or not compatible to the current version of the application`, e il rifiuto non dipende dal file: colpisce un diffusore singolo e un line array del database del 2016 allo stesso modo, e colpisce anche un modello del 2022 che l'installatore stesso del programma ha depositato nel prefix, cioè un file del produttore per questa esatta versione. Quest'ultimo è il dato che esclude sia l'età del database sia la versione del formato.

Quattro cause sono state escluse per misura e non per opinione. Non è un modulo mancante, perché il registro di Wine non riporta alcun errore di caricamento nella finestra temporale dell'operazione. Non è il servizio di database, che è vivo e in ascolto sull'indirizzo locale. Non sono i file, che portano la firma `EGLL` e dimensioni plausibili. E non è la libreria dei certificati, perché sostituirla con quella originale di Windows non cambia nulla.

Una quinta causa è stata esclusa lo stesso giorno, ed era la più promettente. Con la traccia del caricamento delle librerie attiva, l'unico evento anomalo nella finestra dell'importazione era `dssenh.dll`, cioè il provider di firma digitale, che Wine carica nella propria versione e che rifiutava i flag richiesti. Sostituito con la libreria originale di Windows, il rifiuto di flag scompare e l'importazione fallisce identica: il provider crittografico non era la causa, e l'ipotesi è ritirata in MS-113. Resta come lezione che un messaggio di funzionalità mancante in un registro di traccia è un candidato e non una causa.

Il candidato ora più forte è la versione di Wine, dato che la macchina ha `wine-10.0` dai repository della distribuzione mentre questa documentazione raccomanda i pacchetti ufficiali di WineHQ; è però una decisione sull'ambiente e non una prova rapida, perché le due provenienze non si mescolano. Restano poi i redistributabili di Visual C++, che continuano a non spiegare l'assenza di errori di caricamento, e una verifica a basso costo non ancora fatta, cioè provare un modello GLL scaricato oggi dal sito del costruttore, che direbbe se il difetto riguardi tutti i modelli o soltanto quelli prodotti prima di una certa data.
...
La prova in interfaccia del 2026-09-15 ha mostrato che la visibilità della cartella, pur misurata, rispondeva a una domanda diversa da quella che conta. Il programma non legge i modelli da un percorso: li importa nella propria base di dati, con la voce di menu `Import System Definition File`, il cui filtro dichiara `System Definition Files (*.gll)`. La cartella su `Z:` resta quindi utile e continua a non andare copiata, ma cambia ruolo: è la sorgente da cui si importa, non la posizione da cui il programma legge. La finestra `Manage System Definitions` conferma la divisione dei compiti, perché offre soltanto rimozione ed esportazione e nessuna importazione. Il comando qui sotto stampa il percorso nella forma che il programma si aspetta, da incollare nella sua finestra di apertura.
