...
La causa probabile, dichiarata come ipotesi forte e non come fatto, è il provider crittografico. Con la traccia del caricamento delle librerie attiva, l'unico evento pertinente nella finestra dell'importazione è `dssenh.dll`, cioè il provider di firma digitale di Windows, che Wine carica nella propria versione e che rifiuta i flag richiesti con `fixme:dssenh:CPAcquireContext unsupported flags f0000008`. La lettura coerente è che il lettore di GLL verifichi la firma del file e che la verifica non si compia. Il candidato successivo è quindi sostituire quel provider con la libreria originale di Windows; il candidato dopo è una versione di Wine più recente di quella dei repository della distribuzione.
