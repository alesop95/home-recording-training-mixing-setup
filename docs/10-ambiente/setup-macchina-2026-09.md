...
## Aggiornamento del 10 settembre: che cosa è cambiato rispetto alla tabella

Due righe della tabella qui sopra sono superate dal lavoro del pomeriggio del 9 settembre e della mattina del 10, e vanno lette insieme a questa sezione invece di essere corrette in tabella: quella fotografia dichiara di essere stata letta il 9 settembre e riscriverla la renderebbe una fotografia di nessun momento. La riga su Wine, che lo dava non installato con `i386` non dichiarata, è superata: l'architettura è dichiarata e `wine32:i386` risulta `ii` alla versione `10.0~repack-12ubuntu1`. La riga sul prefix vecchio, che lo dava presente e non ancora usato, è superata: il prefix `~/.wine` è stato aperto, migrato da Wine 9 a Wine 10 dopo una copia di sicurezza da 831 MB, e contiene AKABAK e VACS funzionanti con la licenza valida.

Sulla licenza il 10 settembre ha prodotto la verifica che la procedura attendeva, e il suo esito cambia il costo della fase 8. L'attivazione non vive nel registro del prefix ma nel file `C:\ProgramData\RDTeam\Akabak.ini`, a livello di macchina, il che spiega perché la migrazione del prefix non l'abbia toccata; il codice scritto là coincide con quello conservato, e la finestra `Release code` del menu di aiuto dichiara `Release Code valid` con il Machine Identifier `C937-FD3A` invariato. Non c'è quindi alcun codice da reinserire, e la sottofase 8.2 si chiude per osservazione. Il dettaglio è in MS-085.

Resta accertata la sola parte che una finestra sola non poteva dare, e va dichiarata come aperta invece di essere risolta per ragionamento: la finestra del release code non nomina l'edizione, mentre la barra del titolo del programma riporta `32 Professional` e le osservazioni del 7 settembre, su cui poggiano tre pagine di questa documentazione, riportavano `Standard Edition`. Le due letture non si conciliano senza aprire la finestra di informazioni sul programma, che è l'unica a dichiarare l'edizione insieme al profilo di Windows e allo spazio di memoria.

## Inventario dei dispositivi audio e MIDI, misurato il 10 settembre

Questa sezione esiste perché l'inventario dei dispositivi è stato la fonte di una affermazione sbagliata sopravvissuta a lungo, cioè la Focusrite Scarlett 2i2 dichiarata come hardware della macchina e ritirata in MS-079. La regola che ne discende è che i dispositivi si elencano con un comando e non si desumono da un ricordo, e i valori qui sotto vengono da `/proc/asound/cards`, `lsusb`, `amidi -l` e `aconnect -l`.

Il sistema vede una sola scheda audio, `HDA Intel PCH`, che è la scheda integrata `ALC887-VD` della piastra madre. Nessuna interfaccia audio esterna è presente, e questa è una conferma indipendente di MS-079 raccolta un giorno dopo. La conseguenza sul progetto è quella già registrata in PA-012: la fase 6 non attende un cavo ma una decisione di acquisto, e da quella decisione dipende quella del microfono, in un ordine che non si può invertire.

Sul MIDI l'utente ha dichiarato di possedere un controller a tastiera Nektar Impact GX49 e di averne collaudato la connessione sulla macchina con esito positivo. La dichiarazione è vera per il sistema precedente e non si estende a questo, perché l'installazione dell'8 settembre ha azzerato la radice, e la misura del 10 settembre lo mostra in modo netto: il dispositivo non è collegato adesso, dato che `lsusb` non ha corrispondenze, `amidi -l` restituisce una tabella vuota e `aconnect -l` elenca soltanto i client di sistema. Lo strato MIDI, invece, è presente e attivo, e PipeWire espone i propri client dichiarandosi `UMP-MIDI2`, cioè con il protocollo MIDI 2.0: non manca nulla da installare, e il collaudo consiste nel collegare il dispositivo e rifare le tre letture.

Il ruolo del controller va delimitato perché appartiene all'altro dei due progetti che condividono questa macchina. Una tastiera di controllo non produce suono da sé e non misura nulla, quindi non entra né nella catena di misura né in quella di simulazione, e nessuna delle otto fasi del flusso di progettazione dei monitor la impiega. Serve al progetto gemello di home recording come dispositivo di ingresso per gli strumenti virtuali. Sta scritta qui, e non solo là, perché questo blocco documentale descrive la macchina condivisa e l'inventario dell'hardware presente è un fatto che serve a entrambi.
