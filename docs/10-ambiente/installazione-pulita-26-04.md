...
Conviene configurare adesso invece di scoprirlo alla prima iterazione della fase 5, e i passi sono i seguenti. Si apre AKABAK nel prefix, si va al menu `Options/Preferences` e si seleziona la pagina `VACS`. Se si lancia da una sessione SSH il comando nudo non basta e il programma non apre alcuna finestra: servono `DISPLAY` e `XAUTHORITY`, e la forma corretta con la spiegazione del perché sta in `wine-troubleshooting.md`, alla scheda sull'errore `no driver could be loaded`. Là si imposta il modo di trasferimento su `Files`, lasciando la cartella di destinazione al default, che è la cartella del progetto. La stessa impostazione resta sovrascrivibile per singola osservazione nella pagina `Range` della sua form, quindi la scelta qui è un default e non un vincolo.

Eseguita e verificata il 2026-09-14, ed è MS-094. La discordanza fra le due pagine della guida è risolta in favore del capitolo dell'appendice: la finestra ha quattro schede, cioè `Graphics`, `Processing`, `VACS` e `Files`, e la scheda `VACS` contiene il controllo `Spectrum way of output` insieme al campo `Folder` e alla casella `Text format`. Il testo informativo stampato dentro quella pagina è però incompleto a sua volta, perché nomina la sola coppia COM e appunti mentre il controllo offre anche i file, e nomina `ABEC` invece di AKABAK perché la finestra è condivisa con il programma gemello dello stesso autore.

Dove finisce l'impostazione, e come si riproduce senza toccare i menu. La scelta si scrive in `AppData\Local\RDTeam\Akabak.ini` sotto `[Program]`, in tre chiavi: `SpectrumOutputType=3`, che è il valore corrispondente a `File`, `SpectrumOutputAsText=0` per la casella non spuntata, e `SpectrumOutputFolder=` lasciata vuota, che significa cartella del progetto. Il file della licenza sotto `ProgramData` non viene toccato, verificato per impronta. Su un prefix nuovo l'impostazione si può quindi scrivere direttamente invece di ripetere il giro di menu.

Controllo di uscita della sottofase, e come è stato eseguito. Si registrano le impronte dei tre file di configurazione del prefix prima dell'azione e si rileggono dopo: deve risultare cambiato il solo `AppData\Local\RDTeam\Akabak.ini`, e devono comparirvi le tre chiavi. Vale la pena sapere che la conferma delle preferenze materializza su disco l'intero insieme delle impostazioni e non la sola voce toccata, quindi il file passa da cinque a ventotto chiavi: non è un effetto collaterale da temere ma un guadagno, perché rende la configurazione del programma un file di testo confrontabile. Un controllo più forte, da fare quando esisterà un progetto da calcolare, è che una osservazione spettrale produca i file nella cartella attesa e che VACS li importi con il menu `IO/Import Data`.
...
Al primo avvio dopo l'installazione del framework il terminale si riempie di centinaia di righe che nominano `mscorsvw.exe` e un errore di COM. Non è un guasto: è il servizio con cui .NET precompila le proprie librerie, che lavora a code avviando un processo per gruppo, e si esaurisce da sé in una manciata di minuti. Va previsto perché altrimenti si interviene per errore interrompendolo, e va verificato contando i processi invece di guardare il terminale, perché un flusso che smette di scorrere può voler dire tanto finito quanto bloccato. La classificazione completa è in MS-106.
...
La verifica in interfaccia del 2026-09-15 ha però mostrato che la cartella non basta, perché il programma non legge i modelli da un percorso: li importa nella propria base di dati con la voce di menu `Import System Definition File`, e la cartella su `Z:` è la sorgente dell'importazione e non la posizione di lettura.

Questa sottofase resta aperta, e il blocco è nuovo. Nessun modello è importabile: il programma rifiuta ogni file con `The following file is no System Definition or not compatible to the current version of the application`, compreso un modello che il suo stesso installatore ha depositato nel prefix. Sono state escluse per misura cinque cause, cioè un modulo mancante, il servizio di database, i file stessi, la libreria dei certificati e il provider di firma digitale `dssenh.dll`, quest'ultimo sostituito con la libreria originale di Windows senza alcun cambiamento nell'esito. Il candidato ora più forte è la versione di Wine, che sulla macchina è quella dei repository della distribuzione invece dei pacchetti ufficiali di WineHQ. Il racconto è in MS-112 e il ritiro dell'ipotesi sul provider in MS-113.
