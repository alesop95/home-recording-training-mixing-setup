...
Questa sottofase resta aperta, e il blocco è nuovo. Nessun modello è importabile: il programma rifiuta ogni file con `The following file is no System Definition or not compatible to the current version of the application`, compreso un modello che il suo stesso installatore ha depositato nel prefix. Sono state escluse per misura quattro cause, cioè un modulo mancante, il servizio di database, i file stessi e la libreria dei certificati. La causa probabile, dichiarata come ipotesi, è il provider di firma digitale `dssenh.dll`, che nella versione di Wine rifiuta i flag richiesti; il candidato successivo è sostituirlo con la libreria originale di Windows. Il racconto è in MS-112, e le due cause escluse insieme alle prove stanno lì.
